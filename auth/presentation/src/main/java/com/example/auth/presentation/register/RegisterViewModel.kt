...
class RegisterViewModel(
    ...
    fun onAction(action: RegisterAction) {
        when (action) {
            ...
            RegisterAction.OnTogglePasswordVisibility -> TODO()
            else -> Unit
        }
    }
        ...
        when (result) {
            is Result.Failure -> {
                if (result.error == DataError.Network.CONFLICT) {
                    eventChannel.send(RegisterEvent.Failure(UiText.StringResource(R.string.error_email_already_exists)))
                } else {
                    eventChannel.send(RegisterEvent.Failure(error = result.error.asUiText()))
                }
            }
            is Result.Success -> {
